...
public class GameEngine {
    ...
    private void processPlayerChoice(int choice, Scanner scanner) throws InvalidProjectException {
        switch (choice) {
    ...
    private void handleHireEmployee(Scanner scanner) {
        System.out.println("\n  ── HIRE EMPLOYEE ───────────────────────────────────");
        ...
        System.out.println("  Tip: max affordable skill = Dev:20, Mgr:14, Tester:33 (with $100k)");
        ...
        if (playerCompany.getCash() < employee.getSalary()) {
            System.out.printf("  ERROR: Not enough funds! Required $%.0f, Available $%.0f. Turn skipped.%n",
                    employee.getSalary(), playerCompany.getCash());
            return;
        }

        try { playerCompany.deductCash(employee.getSalary()); }
        catch (InsufficientFundsException e) { System.out.println("  ERROR: " + e.getMessage()); return; }
    ...
    private void handleStrategicProject(Scanner scanner) throws InvalidProjectException {
        System.out.println("\n  ── START PROJECT ───────────────────────────────────");
        ...
        try { playerCompany.deductCash(budget); }
        catch (InsufficientFundsException e) { System.out.println("  ERROR: " + e.getMessage()); return; }
